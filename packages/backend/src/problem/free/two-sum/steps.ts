...
export function generateSteps(nums: number[], target: number): ProblemState[] {
  ...
  l.breakpoint_explanation = "Initial state: target, nums array, and empty 'seen' map.";
  l.breakpoint(1); // Corresponds to #1

  for (let i = 0; i < nums.length; i++) {
    const num = nums[i]; // num defined here to be used in explanation for breakpoint 2
    l.breakpoint_explanation = `Start of loop iteration for nums[${i}] (${num}).`;
    ...
    l.breakpoint_explanation = `Calculated complement = target (${target}) - nums[${i}] (${num}) = ${complement}.`;
    ...
    if (existsInSet) {
      ...
      l.breakpoint_explanation = `Complement (${complement}) found in 'seen' map at index ${complementIndex}. Result is [${complementIndex}, ${i}]. Returning.`;
      l.breakpoint(4); // Corresponds to #4 Found complement
      return l.getSteps();
    }
    l.breakpoint_explanation = "Complement not found in 'seen' map yet.";
    ...
    l.breakpoint_explanation = `Added nums[${i}] (${num}) and its index ${i} to 'seen' map.`;
    l.breakpoint(6); // Corresponds to #6 Add current number and index to map
  }
  l.breakpoint_explanation = "Loop finished.";
  ...
  l.breakpoint_explanation = "No solution found. Returning empty result.";
