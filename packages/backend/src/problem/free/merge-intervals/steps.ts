...
export function generateSteps(intervals: number[][]) {
  ...
  l.comment = "Initial state: intervals before sorting.";
  l.breakpoint(1);
  ...
  l.comment = "Intervals sorted. First interval added to merged list.";
  ...
  for (let i = 1; i < intervals.length; i++) {
    ...
    l.comment = `Comparing currentInterval [${currentInterval.join(
      ","
    )}] with lastMerged interval [${lastMerged.join(",")}].`;
    l.breakpoint(3);
    ...
    if (currentStart <= lastEnd) {
      ...
      l.comment = `Overlap found. Merged currentInterval into lastMerged. Updated lastMerged: [${lastMerged.join(
        ","
      )}].`;
      l.breakpoint(4);
    } else {
      ...
      l.comment = `No overlap. Added currentInterval [${currentInterval.join(
        ","
      )}] to merged list.`;
      l.breakpoint(5);
    }
  ...
  l.comment = "Final result: merged intervals.";
  l.breakpoint(6);
