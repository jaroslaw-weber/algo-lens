...
export function generateSteps(nums: number[]): ProblemState[] {
  ...
  logger.breakpoint_explanation = "Initial state: nums, productsLeft, productsRight, and output arrays (all 1s).";
  logger.breakpoint(1);
  ...
  for (let i = 1; i < length; i++) {
    ...
    logger.arrayV2({ productsRight: productsRight }, {});
    logger.arrayV2({ output: output }, {});
    logger.breakpoint_explanation = `Calculated productsLeft[${i}] = productsLeft[${i-1}] (${prevProductsLeft}) * nums[${i-1}] (${prevNum}) = ${productsLeft[i]}.`;
    logger.breakpoint(2);
  }
  ...
  for (let i = length - 2; i >= 0; i--) {
    ...
    logger.arrayV2({ output: output }, {});
    logger.breakpoint_explanation = `Calculated productsRight[${i}] = productsRight[${i+1}] (${nextProductsRight}) * nums[${i+1}] (${nextNum}) = ${productsRight[i]}.`;
    logger.breakpoint(3);
  }
  ...
  for (let i = 0; i < length; i++) {
    ...
    logger.breakpoint_explanation = `Calculated output[${i}] = productsLeft[${i}] (${currentProductsLeft}) * productsRight[${i}] (${currentProductsRight}) = ${output[i]}.`;
    logger.breakpoint(4);
  }
  ...
  logger.hide("output");
  logger.breakpoint_explanation = "Final result: output array.";
  logger.breakpoint(5);
