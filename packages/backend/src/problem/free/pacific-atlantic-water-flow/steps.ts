import { Pointer2D, ProblemState } from "algo-lens-core"; // Import Pointer2D
...
export function generateSteps(heights: number[][]): ProblemState[] {
  ...
  while (pacificQueue.length > 0) {
    ...
    l.comment = `Process Pacific queue cell. Explore neighbors.`;
  ...
  while (atlanticQueue.length > 0) {
    ...
    l.comment = `Process Atlantic queue cell. Explore neighbors.`;
