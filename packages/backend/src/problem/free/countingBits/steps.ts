...
export function generateSteps(n: number): ProblemState[] {
  ...
  for (let i = 0; i <= n; i++) {
    ...
    l.arrayV3({ result: result }, { "i - 1": i - 1 }); // Highlight previous result entry if exists
    ...
    while (inner_num > 0) {
      l.arrayV3({ result: result }, { "i - 1": i - 1 });
    ...
    l.arrayV3({ result: result }, { "i - 1": i - 1 }); // Show previous state
    ...
    l.arrayV3({ result: result }, { i: i }); // Highlight the newly added result
