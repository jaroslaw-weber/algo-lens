...
const tripletInfoGroup = groups.find(g => g.name === 'tripletInfo')?.name || 'tripletInfo';
const pointersGroup = groups.find(g => g.name === 'pointers')?.name || 'pointers';
...
export function generateSteps(p: ThreeSumInput): ProblemState[] {
  ...
  for (let i = 0; i < nums.length - 2; i++) {
    ...
    while (left < right) {
      ...
      l.breakpoint(6, `Inner loop: Calculate sum = ${sum} for triplet [${triplet.join(',')}]`);
      l.array("nums", nums, i, left, right);
      l.simple({ target });
      l.group(pointersGroup, { i, left, right });
      ...
      if (sum === target) {
        ...
        l.breakpoint(7, `Inner loop: Found potential triplet [${triplet.join(',')}] with sum = ${target}`);
        l.array("nums", nums, i, left, right);
        l.simple({ target });
        l.group(pointersGroup, { i, left, right });
        l.group(tripletInfoGroup, { triplet, sum });
        ...
        if (!seen.has(tripletKey)) {
          seen.add(tripletKey);
          result.push(triplet);
          l.breakpoint(8, `Inner loop: Added unique triplet [${triplet.join(',')}] to result`);
          l.array("nums", nums, i, left, right);
          l.simple({ target });
          l.group(pointersGroup, { i, left, right });
          l.group(tripletInfoGroup, { triplet, sum });
        ...
        if (skippedLeft) {
             l.breakpoint(9, `Inner loop: Skipped duplicate left pointers, new left = ${left}`);
             l.array("nums", nums, i, left, right);
             l.simple({ target });
             l.group(pointersGroup, { i, left, right });
        ...
        while (left < right && nums[right] === nums[right - 1]) {
         ...
         if (skippedRight) {
             l.breakpoint(10, `Inner loop: Skipped duplicate right pointers, new right = ${right}`);
             l.array("nums", nums, i, left, right);
             l.simple({ target });
             l.group(pointersGroup, { i, left, right });
             l.group(tripletInfoGroup, { triplet, sum }); // Keep old triplet info for context
        ...
        l.breakpoint(11, `Inner loop: Moving pointers after finding target sum`);
        l.array("nums", nums, i, left, right);
        l.simple({ target });
        l.group(pointersGroup, { i, left, right });
        l.group(tripletInfoGroup, { triplet, sum });
        ...
        l.breakpoint(12, `Inner loop: Moved pointers, new left = ${left}, new right = ${right}`);
        l.array("nums", nums, i, left, right);
        l.simple({ target });
        l.group(pointersGroup, { i, left, right });
        ...
        l.breakpoint(13, `Inner loop: Sum ${sum} < target ${target}, incrementing left pointer`);
        l.array("nums", nums, i, left, right);
        l.simple({ target });
        l.group(pointersGroup, { i, left, right });
        l.group(tripletInfoGroup, { triplet, sum });
        ...
        l.breakpoint(14, `Inner loop: Sum ${sum} > target ${target}, decrementing right pointer`);
        l.array("nums", nums, i, left, right);
        l.simple({ target });
        l.group(pointersGroup, { i, left, right });
        l.group(tripletInfoGroup, { triplet, sum });
