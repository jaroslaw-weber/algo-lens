...
export function sameTree(
  ...
  function checkNodes(
    ...
    l.comment = `Comparing nodes pNode (${
      pNode ? pNode.val : "null"
    }) and qNode (${qNode ? qNode.val : "null"}).`;
    ...
    if (!pNode && !qNode) {
      ...
      l.comment =
        "Base case: Both pNode and qNode are null. Trees are same at this path. Returning true.";
    ...
    if (!pNode || !qNode) {
      ...
      l.comment = `Base case: One of pNode or qNode is null, the other is not. pNode is ${
        pNode ? pNode.val : "null"
      }, qNode is ${
        qNode ? qNode.val : "null"
      }. Trees are different. Returning false.`;
    ...
    if (pNode.val !== qNode.val) {
      ...
      l.simple({ "is node same?": false });
      l.comment = `Nodes have different values (pNode.val = ${pNode.val}, qNode.val = ${qNode.val}). Trees are different. Returning false.`;
    ...
    if (!leftSame) {
      ...
      l.comment = `Recursive step result for node pNode (${pNode.val}): Node values matched. Left subtree same: ${leftSame}. Returning false.`;
      l.breakpoint(5);
      return false;
    }
    ...
    l.comment = `Recursive step result for node pNode (${pNode.val}): Node values matched. Left subtree same: ${leftSame}. Right subtree same: ${rightSame}. Overall result for this node: ${result}.`;
  ...
  l.simple({ result });
  l.comment = `Overall result for the entire tree: ${result}.`;
