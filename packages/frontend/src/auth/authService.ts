...
export const authService = {
     ...
     */
    async register(userData: UserCreateParams): Promise<RecordAuthResponse<RecordModel>> {
     ...
     * @returns A promise that resolves with the authentication response.
     */
    async login(credentials: UserLoginParams): Promise<RecordAuthResponse<RecordModel>> {
        return await pb.collection('users').authWithPassword(credentials.identity, credentials.password);
    },
     ...
     */
    onAuthStateChange(callback: (token: string, model: RecordModel | null) => void): () => void {
